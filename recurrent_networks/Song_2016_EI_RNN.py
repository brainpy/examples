...
class RNN(bp.Base):
  ...
  def __init__(self, num_input, num_hidden, num_output, num_batch,
    ...
    # input weight
    self.w_ir = bm.Variable(bp.init.init_param(w_ir, (num_input, num_hidden)))

    # recurrent weight
    bound = 1 / num_hidden ** 0.5
    self.w_rr = bm.Variable(bp.init.init_param(w_rr, (num_hidden, num_hidden)))
    ...
    self.w_ro = bm.Variable(bp.init.init_param(w_ro, (self.e_size, num_output)))
